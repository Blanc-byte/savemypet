        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 style="font-size: 20px; font-weight: bold; margin-bottom: 20px;">List Of Rooms</h3>
                    
                    <button onclick="openAddDoctorModal()" class="assign-button" style="margin-bottom: 20px;">Add Room</button>
                            ...
                            <tbody>
                                @foreach($rooms as $room)
                                    <tr>
                                        <td>{{ $room->id }}</td>
                                        <td>{{ $room->name }}</td>
                                        <td class="concern-cell">{{ $room->status }}</td>
                                        <td>
                                            <button class="assign-button" onclick="openEditDoctorModal('{{ $room->id }}', '{{ $room->name }}', '{{ $room->status }}')">Edit</button>
                                            <form action="{{ route('room.destroy', $room->id) }}" method="POST" id="delete-form-{{ $room->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button class="assign-button4 ab" onclick="event.preventDefault(); openConfirmationModal2(document.getElementById('delete-form-{{ $room->id }}'))">
                                                    DELETE
                                                </button>
                                            </form>
                                        </td>
...
<div id="confirmation-modal" class="modal-overlay hidden">
    <div class="confirmation-modal-content">
        <h2 class="text-xl font-bold mb-4">Are you sure you want to remove this Room?</h2>
...
<div id="edit-doctor-modal" class="modal-overlay hidden">
    <div class="modal-content">
        ...
        <form id="edit-doctor-form">
            @csrf
            <div class="mb-4">
                <label for="doctor-name" class="block">Name</label>
...
<div id="add-doctor-modal" class="modal-overlay hidden">
    <div class="doctor-modal-content">
        ...
        <form action="{{ route('room.add') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="add-doctor-name" class="block">Name</label>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" class="assign-button ml-2">Add Room</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function openEditDoctorModal(doctorId, name, status) {
        appID = doctorId;
